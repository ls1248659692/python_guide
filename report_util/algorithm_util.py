...
def bubble_sort(input_list, ascending=True):
    '''
    函数说明:冒泡排序（默认顺序为升序）
    '''
    if len(input_list) == 0:
    ...
    range_num = len(sorted_list) - 1

    for i in range(range_num):
        change = False
        for j in range(range_num):
            if sorted_list[j + 1] < sorted_list[j] and ascending:
                sorted_list[j], sorted_list[j + 1] = sorted_list[j + 1], sorted_list[j]
                change = True
            elif sorted_list[j + 1] > sorted_list[j] and not ascending:
                sorted_list[j], sorted_list[j + 1] = sorted_list[j + 1], sorted_list[j]
                change = True
        if not change:
            break
...
def insert_sort(input_list, ascending=True):
    '''
    函数说明:直接插入排序（升序）
    '''
    if len(input_list) == 0:
    ...
    for i in range(1, len(sorted_list)):
        temp = sorted_list[i]
        j = i - 1
        while j >= 0 and temp < sorted_list[j] and ascending:
            sorted_list[j + 1] = sorted_list[j]
            j -= 1
        sorted_list[j + 1] = temp

        print  (temp, sorted_list)
...
def binary_insert_sort(input_list):
    ...
    for i in range(1, len(input_list)):
        j = i - 1
        temp = sorted_list[i]
        insert_index = binary_search(sorted_list, i, sorted_list[i])
        if insert_index != -1:
...
def HeadSort(input_list):
    ...
    def HeadAdjust(input_list, parent, length):
        ...
        while child < length:
            if child + \
                    1 < length and input_list[child] < input_list[child + 1]:
                child += 1
...
if __name__ == '__main__':
    input_list = [1, 2, 3, 4, 5, 6, 7, 8, 10, 9]
    print ('before sort list:', input_list)
    # sorted_list = bubble_sort(input_list)
    # sorted_list = insert_sort(input_list)
    # sorted_list = binary_insert_sort(input_list)
    # sorted_list = shell_sort(input_list)
    sorted_list = select_sort(input_list)
    print ('after sort list:', sorted_list)
